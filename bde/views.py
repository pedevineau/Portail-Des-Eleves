...
def voter(request):
    ...
    if listes.count() >= 2: #On peut voter
        peut_voter = True
        fin_vote = (Liste.objects.filter(fin_vote__gte = datetime.datetime.now())[0]).fin_vote
        liste1 = listes[0]
        liste2 = listes[1]        
        if Vote.objects.filter(eleve = request.user.profile).exists(): #L'élève a déjà voté
            ...
            peut_voter = False
            deja_vote = True
    return render(request, 'bde/voter.html', {'peut_voter':peut_voter, 'deja_vote':deja_vote, 'liste1':liste1, 'liste2':liste2, 'debut_vote':debut_vote, 'fin_vote':fin_vote})    
@login_required
...
def resultats(request):
    ...
    return render(request, 'bde/resultats.html', {'liste1':liste1, 'liste2':liste2, 'n_votes_2':n_votes_2, 'n_votes_1':n_votes_1})
@login_required
def palums(request):
    """
        La page des Palums
    ...
    return render(request, 'bde/palums.html', {'liste_palums_1A': liste_palums_1A, 'liste_palums_2A': liste_palums_2A, 'liste_palums_3A': liste_palums_3A})
@login_required
...
def offre_stage(request):
    return render(request, 'bde/offre_stage.html', {})
@login_required
def voeux_parrainage(request):
    parrain = request.user.profile
    ...
    return render(request, 'bde/voeux_parrainage.html', {'form': form,'dejaVote':dejaVote,'deuxA':deuxA, 'votesOuverts':votesOuverts})
@login_required
def visualiser_voeux_parrainage(request):
    ...
    if dejaVote:
        ...
        return render(request, 'bde/visualiser_voeux_parrainage.html', {'dejaVote':dejaVote,'deuxA':deuxA,'fillot1':fillot1,'fillot2':fillot2,'fillot3':fillot3,'fillot4':fillot4,'fillot5':fillot5,'fillot6':fillot6,'fillot7':fillot7,'fillot8':fillot8})    
    return render(request, 'bde/visualiser_voeux_parrainage.html', {'dejaVote':dejaVote,'deuxA':deuxA})
